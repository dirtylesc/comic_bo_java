...
@Repository
public interface ViUserRepository extends JpaRepository<ViTranslators, Long> {
    ...
    @Query(value = "select ta_u.id, ta_u.name, ta_u.nickname " +
            ...
            "and ta_t.user_id = :curUserId", nativeQuery = true)
    List<ViTranslators> getAllTranslators(@Param("q") String q, @Param("role") Integer role, @Param("curUserId") Long userId);
}
